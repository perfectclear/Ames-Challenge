...
#    print('acc_Ada_Boost'+str(acc_Ada_Boost))
    rmse_train = rmse_cv_train(XGBoost).mean()
    rmse_test  = rmse_cv_test(XGBoost).mean()
#    print('actual_accuracy_Ada_Boost'+str(actual_accuracy))
    rmse_xgb_train.append(rmse_train)
    rmse_xgb_test.append(rmse_test)
    print("{} fold accuracy (train) (XGBoost): {}".format(i+1, rmse_xgb_train[-1]))
    print("{} fold accuracy (test) (XGBoost): {}".format(i+1, rmse_xgb_test[-1]))
    xgb_predictions.append(y_pred)
print("Mean accuracy (train) (XGBoost): ", np.mean(rmse_xgb_train))   
print("Mean accuracy (test) (XGBoost): ", np.mean(rmse_xgb_test))   
xgb_submission = xgb_predictions.mean()

#stack ensemble
lass = LassoCV(alphas = [0.0001, 0.0003, 0.0006, 0.001, 0.003, 0.006, 0.01, 0.03, 0.06, 0.1, 
                          0.3, 0.6, 1],  cv = 10)
eclf = StackingRegressor(regressors=[lasso, ridge], meta_regressor = lass)
eclf.fit(X_train, y_train)
#alpha = eclf.alpha_
#print("Best alpha :", alpha)
#
#print("Try again for more precision with alphas centered around " + str(alpha))
#lass = LassoCV(alphas = [alpha * .6, alpha * .65, alpha * .7, alpha * .75, alpha * .8, 
#                          alpha * .85, alpha * .9, alpha * .95, alpha, alpha * 1.05, 
#                          alpha * 1.1, alpha * 1.15, alpha * 1.25, alpha * 1.3, alpha * 1.35, 
#                          alpha * 1.4], 
#                max_iter = 50000, cv = 10)
#eclf = StackingRegressor(regressors=[lasso, ridge, random_forest], meta_regressor = lass )
#eclf.fit(X_train, y_train)
#alpha = eclf.alpha_
#print("Best alpha :", alpha)
print("eclf RMSE on Training set :", rmse_cv_train(eclf).mean())
print("eclf RMSE on Test set :", rmse_cv_test(eclf).mean())
y_train_eclf = eclf.predict(X_train)
y_test_eclf = eclf.predict(X_test)


eclf_submission = eclf.predict(test)

# Plot residuals
plt.scatter(y_train_eclf, y_train_eclf - y_train, c = "blue", marker = "s", label = "Training data")
plt.scatter(y_test_eclf, y_test_eclf - y_test, c = "lightgreen", marker = "s", label = "Validation data")
plt.title("Linear regression with eclf regularization")
plt.xlabel("Predicted values")
plt.ylabel("Residuals")
plt.legend(loc = "upper left")
plt.hlines(y = 0, xmin = 10.5, xmax = 13.5, color = "red")
...
# Plot predictions
plt.scatter(y_train_eclf, y_train, c = "blue", marker = "s", label = "Training data")
plt.scatter(y_test_eclf, y_test, c = "lightgreen", marker = "s", label = "Validation data")
plt.title("eclf")
plt.xlabel("Predicted values")
plt.ylabel("Real values")
plt.legend(loc = "upper left")
...
#          'Ensemble']
#
#for clf, lab, grd in zip([ridge, lasso, random_forest, eclf],
#                         labels,
...
eclf_submission = (3*rdg_submission+ las_submission+ eclf_submission)/5
...
eclf_submission = np.exp(eclf_submission)
eclf_submission = pd.DataFrame({
        "Id": ID,
        "SalePrice": eclf_submission
    })
eclf_submission.to_csv('eclf_submission.csv', index=False)
