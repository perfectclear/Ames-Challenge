...
for i, (train_index, test_index) in enumerate(indicies):
    ...
    print('acc_XGBoost'+str(acc_XGBoost))
...
#    print('actual_accuracy_Ada_Boost'+str(actual_accuracy))
    acc_xgb_train.append(acc_XGBoost)
    print("{} fold accuracy (train) (XGBoost): {}".format(i+1, acc_xgb_train[-1]))
    xgb_predictions.append(y_pred)
print("Mean accuracy (train) (XGBoost): ", np.mean(acc_xgb_train))    

xgb_submission = np.mean(xgb_predictions, axis=0)
...
xgb_submission = np.exp(xgb_submission)
xgb_submission = pd.DataFrame({
        "Id": ID,
        "SalePrice": xgb_submission
    })
